...
class AVPlayer {
  public:
    AVPlayer(const std::string &url, Canvas *canvas, AsyncTask *async_task)
        ...
        this->_video_timer = async_task->do_timer_for(
            [this](auto &, auto timer) { this->_handle_video_frame(timer); },
            std::chrono::milliseconds(0));
        this->_audio_timer = async_task->do_timer_for(
    ...
    ~AVPlayer() {
        ...
        this->_video_timer->cancel();
        this->_video_timer->wait();

        this->_audio_timer->cancel();
        this->_audio_timer->wait();
  ...
  private:
    ...
    std::shared_ptr<boost::asio::steady_timer> _video_timer{};
    std::shared_ptr<boost::asio::steady_timer> _audio_timer{};
    ...
    void _handle_video_frame(std::shared_ptr<boost::asio::steady_timer> timer) {
...

        {
            ...
            this->_cv.wait(l_lock, [this]() { return this->_clock != -1; });
        }
